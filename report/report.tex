...
Unicode letters from all alphabets are allowed and emoji characters are also allowed as letters.
...
\begin{lstlisting}[style=example]
    2020 0DH 12.3 4.567E8 0.55712566D-6 2.3e-10
\end{lstlisting}

\subsection{\BOOLEAN}

\BOOLEAN\ constants are \TRUE\ and \FALSE. \label{subsec:bool-consts}
...
Also C++ style line comments are supported, starting with \lstinline!//! and continuing to the end of the line.
...
Declarations also serve to specify certain permanent properties of an object, such as whether it is a constant, a type, a variable, or a procedure.
...
An identifier declared in a module block may be followed by an export mark (\lstinline!*! or \lstinline!-!) in its declaration to indicate that it is exported.
An identifier \lstinline!x! exported by a module \lstinline!M! may be used in other modules, if they import \lstinline!M! (see \ref{modules}).
The identifier is then denoted as \lstinline!M.x! in these modules and is called a qualified identifier.
Variables and record fields marked with \lstinline!-! in their declaration are read-only in importing modules.
...
Examples of \INTEGER, \CHAR, and Enumeration \CASE\ statements:
\begin{lstlisting}[style=example]
...
\begin{tabular}{lp{4.5cm}p{6cm}}
    ...
    \DEC(e) & enumeration type & previous enumeration value \\ % Runtime
    ...
    \INC(e) & enumeration type & next enumeration value \\  % Runtime
