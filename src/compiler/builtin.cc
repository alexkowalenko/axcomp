...
BIFunctor newfunct{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
    ...
    throw CodeGenException(ast->get_location(), "Variable with type {0} passed to NEW",
    ...
    if (auto *arg = args[0]; arg->getType()->isPointerTy()) {
        ...
        if (args.size() == 1) {
            ...
            if (args[1]->getType()->isIntegerTy()) {
                ...
                throw CodeGenException(ast->get_location(), "Type {0} passed to {1} as increment",
...
BIFunctor long_func{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
    ...
    throw CodeGenException(ast->get_location(), "Type {0} passed to LONG",
                           ast->args[0]->get_type()->get_name());
}};
