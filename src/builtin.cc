...
BIFunctor abs{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  if (arg->getType()->isFloatingPointTy()) {
    ...
    auto *fun = Intrinsic::getDeclaration(codegen->get_module().get(),
                                          Intrinsic::fabs, type_args);
...
BIFunctor len{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  if (arg->getType()->isPointerTy()) {
    return codegen->call_function("Strings_Length",
                                  TypeTable::IntType->get_llvm(), {arg});
  }
  return TypeTable::IntType->make_value(1);
}};
...
BIFunctor newfunct{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  if (ast->args.size() == 1 &&
      ast->args[0]->get_type()->id == TypeId::pointer) {
    debug("builtin NEW POINTER");
    auto ptr_type =
        std::dynamic_pointer_cast<ax::PointerType>(ast->args[0]->get_type());
    auto size = ptr_type->get_reference()->get_size();
    return codegen->call_function(
        "NEW_ptr", TypeTable::IntType->get_llvm(),
        {args[0], TypeTable::IntType->make_value(size)});
  }
  if (ast->args.size() > 1 &&
      ast->args[0]->get_type()->id == TypeId::openarray) {
    debug("builtin NEW ARRAY");

    auto array_type =
        std::dynamic_pointer_cast<ArrayType>(ast->args[0]->get_type());
    ...
    auto *ptr =
        codegen->call_function("NEW_Array", array_type->get_llvm(), {value});
    return codegen->get_builder().CreateStore(ptr, args[0]);
  }
  throw CodeGenException(std::format("Variable with type {0} passed to NEW",
...
BIFunctor max{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  if (!type) {
    throw CodeGenException(
        std::format("{0}: {1} is not a type name", max_f ? "MAX" : "MIN", name),
        ast->get_location());
  }
...
BIFunctor inc{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  if (arg->getType()->isPointerTy()) {
    ...
    if (args.size() == 1) {
      ...
      if (args[1]->getType()->isIntegerTy()) {
        ...
        throw CodeGenException(
            std::format("Type {0} passed to {1} as increment",
  ...
  throw CodeGenException(std::format("Type {0} passed to {1}",
                                     ast->args[0]->get_type()->get_name(),
                                     inc_f ? "INC" : "DEC"),
                         ast->get_location());
}};
...
BIFunctor floor{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  std::vector<llvm::Type *> type_args{TypeTable::RealType->get_llvm()};
  auto *fun = Intrinsic::getDeclaration(codegen->get_module().get(),
                                        Intrinsic::floor, type_args);
  auto *value = codegen->get_builder().CreateCall(fun, args);
  return codegen->get_builder().CreateFPToSI(value,
                                             TypeTable::IntType->get_llvm());
}};

BIFunctor flt{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  auto args = codegen->do_arguments(ast);
  return codegen->get_builder().CreateSIToFP(args[0],
                                             TypeTable::RealType->get_llvm());
}};
...
BIFunctor assert{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  BasicBlock *assert_block =
      BasicBlock::Create(codegen->get_context(), "assert", funct);
...
BIFunctor long_func{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  ...
  throw CodeGenException(std::format("Type {0} passed to LONG",
                                     ast->args[0]->get_type()->get_name()),
                         ast->get_location());
}};
...
BIFunctor incl{[](CodeGenerator *codegen, ASTCall const &ast) -> Value * {
  auto args = codegen->do_arguments(ast);
  auto *set = args[0];
  auto *index = codegen->get_builder().CreateShl(
      TypeTable::IntType->make_value(1), args[1]);
...
void Builtin::initialise(SymbolFrameTable &symbols) {
  ...
  global_functions = {
      ...
      // Maths
      {"ABS",
       Symbol{std::make_shared<ProcedureType>(
                  TypeTable::AnyType,
      ...
      {"ODD", Symbol{std::make_shared<ProcedureType>(
                         TypeTable::BoolType,
                         ProcedureType::ParamsList{
                             {TypeTable::IntType, Attr::null},
                         }),
                     Attr::global_function}},
      {"FLOOR", Symbol{std::make_shared<ProcedureType>(
                           TypeTable::IntType,
                           ProcedureType::ParamsList{
                               {TypeTable::RealType, Attr::null},
                           }),
                       Attr::compile_function}},
      {"ENTIER", Symbol{std::make_shared<ProcedureType>(
                            TypeTable::IntType,
                            ProcedureType::ParamsList{
                                {TypeTable::RealType, Attr::null},
                            }),
      ...
      {"FLT", Symbol{std::make_shared<ProcedureType>(
                         TypeTable::RealType,
                         ProcedureType::ParamsList{
                             {TypeTable::IntType, Attr::null},
                         }),
      ...
      {"WriteLn", Symbol{std::make_shared<ProcedureType>(
      ...
      {"NEW", Symbol{std::make_shared<ProcedureType>(
                         TypeTable::VoidType,
                         ProcedureType::ParamsList{
      ...
      {"COPY", Symbol{std::make_shared<ProcedureType>(
                          TypeTable::VoidType,
                          ProcedureType::ParamsList{
